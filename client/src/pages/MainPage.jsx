import { Autocomplete, Box, Button, Container, TextField, Typography, CircularProgress } from '@mui/material'
...
const MainPage = () => {
  ...
  const [returnDate, setReturnDate] = useState(dayjs('2022-04-22'));
  ...
  const search = () => {
    ...
    const convertedReturnDate = dayjs(returnDate).format('YYYY-MM-DD');

    fetch(`http://localhost:8080/main_search?departureDate=${convertedDepartDate}` +
      `&returnDate=${convertedReturnDate}&departureCity=${depart.code}` +
      `&destinationCity=${dest.code}`
    )
      .then(res => res.json())
      .then(resJson => {
          console.log(`http://localhost:8080/main_search?departureDate=${convertedDepartDate}` +
      `&returnDate=${convertedReturnDate}&departureCity=${depart.code}` +
      `&destinationCity=${dest.code}`)
        setData(resJson);
      });
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyItems: 'center' }}>
      ...
      <Box>
        {data != null ? <FlightTicketsTable flightData={data} /> : null}
      </Box>
    </Box>
